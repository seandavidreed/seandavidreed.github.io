...
I forgot how much I enjoy writing proofs! Here's an inductive proof of the closed form of arithmetic sequence summation:

Prove $\displaystyle\sum_{i=0}^{n-1} (a + di) = an + d(\frac{n(n-1)}{2}$, where $d \in \Z^+$.

Base Case: $n=1$  

$\displaystyle\sum_{i=0]^{0} (a + di) = a \cdot 1 + d \cdot 0$

Inductive Step: $\displaystyle\sum_{i=0}^{k-1} (a + di) = ak + d \cdot \frac{k(k-1)}{2} \implies \displaystyle\sum_{i=1}^{k} (a + di) = a(k+1) + d \cdot \frac{(k+1)k}{2}$

$$
\begin{aligned}
\displaystyle\sum_{i=1}^{k} (a + di) &= \displaystyle\sum_{i=1}^{k} (a + di) \\
&= a + dk + \displaystyle\sum_{i=1}^{k-1} (a + di) \\
&= a + dk + ak + d \cdot \frac{k(k-1)}{2} \\
&= a(k+1) + d[k + \frac{k(k-1)}{2}] \\
&= a(k+1) + d(\frac{k^2 - k + 2k}{2}) \\
&\displaystyle\sum_{i=1}^{k} (a + di) = a(k+1) + d(\frac{k(k+1)}{2}) \blacksquare
$$
