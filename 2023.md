...
With more experimentation on Roboflow, I achieved a _75% mAP_, a great boost to my confidence in this project! However, as I examined the results, I started to realize that my labeling system was not the one for the job; sure, the model could correctly identify a green sticker on the corner facing up, but which corner? Initially, I thought the instance segmentation would be performed in an order that I specified. However, the results were always presented based on confidence levels, so my labels could appear in any order! This would make the project completely fail at the purpose for which I was building it. Once again, I had to go through the painstaking process of relabeling everything. This time, I proceeding in a spiral pattern, labeling each color according to its location in the pattern, _G\_1, R\_7, O\24_. No matter what confidence levels are achieved, the labels themselves will tell me how the cubes are positioned.

As for the _fastai_ problem mentioned in the previous entry, I decided to go another route for now; I'm using the _ultralytics_ library