...
>On average, $\frac{(n + 1)}{2}$ elements must be checked, which we calculate with $\frac{1}{n}\displaystyle\sum_{k=1}^{n}n$. For the worst case, we must all check all the elements, so $n$ elements. These figures expressed in $\theta-notation$ are $\theta(n)$ for the average case and $\theta(n)$ for the worst case.
